...
document.getElementById("calculateButton").addEventListener("click", function () {
  ...
  let ultimateGoalWeight = parseFloat(document.getElementById("ultimateGoalWeight").value);
  ...
  const calculateWeekly = () => {
    ...
    document.getElementById("result").innerHTML = `<h3>Calculating... Week ${totalWeeks}</h3>`;
  ...
  const finalizeChart = () => {
    ...
    resultHTML += `<p><strong>GW:</strong> ${system === "imperial" ? (goalWeight * 2.205).toFixed(2) : goalWeight.toFixed(2)} ${system === "imperial" ? "lb" : "kg"}</p>`;
resultHTML += `<p>Fixed daily calorie intake: <strong>${Math.round(fixedDailyCalories)} kcal</strong></p>`;
resultHTML += `<p><strong>UGW:</strong> ${system === "imperial" ? (ultimateGoalWeight * 2.205).toFixed(2) : ultimateGoalWeight.toFixed(2)} ${system === "imperial" ? "lb" : "kg"}</p>`;
    
    resultHTML += `<p>Estimated time to reach UGW: <strong>${totalWeeks * 7} days</strong></p>`;
...
for (let i = 0; i < weeklyDates.length; i++) {
  resultHTML += `<tr>
    <td>${i + 1}</td>
    <td>${weeklyDates[i]}</td>
    <td>${system === "imperial" ? (weeklyWeights[i] * 2.205).toFixed(2) : weeklyWeights[i]}</td>
    <td>${weeklyTDEEs[i]}</td>
  </tr>`;
}
